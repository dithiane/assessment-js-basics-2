...
const canWeDeliver = (zipCode) => callDeliveryCheck(callIncludes, zipCode)
...
const canWeDeliverTwo = (zipCode) => callDeliveryCheck(callFind, zipCode) 
...
deals.map(el => el.title.includes(replaceWord) ? el.title = el.title.replace(replaceWord, "10") : el)
...
deals.map(el => el.desc.includes(replaceWord) ? el.desc = el.desc.replace(replaceWord, "April").trim() : el)
